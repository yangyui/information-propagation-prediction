...
class TransformerBlock(nn.Module):
    def __init__(self, input_size, d_model=64, n_heads=8, is_layer_norm=True, is_FFN = True, is_future = False, attn_dropout=0.1):
...
class Long_term_atention(nn.Module):
    ...
    def multi_head_attention(self, Q, K, V, mask):
        ...
        bsz, v_len, _ = V.size()
        V = V.cuda()
        Q_ = Q.matmul(self.W_q.cuda())
        K_ = K.matmul(self.W_k.cuda())
        V_ = V.matmul(self.W_v.cuda())
        mask = mask.cuda()  # 确保mask也在GPU上
        V_att = self.scaled_dot_product_attention(Q_, K_, V_, mask)
        return V_att
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        # print("V_att.device:", V_att.device)
