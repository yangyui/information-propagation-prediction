...
from dataLoader import Options, Split_data_1
...
def ConHyperGraphList(cascades, timestamps, user_size, step_split=Constants.step_split):
    ...
    for x in range(split_length, split_length * step_split, split_length):
        if x == split_length:
            sub_graph = torch.where(times > 0, one_vec, zero_vec) - torch.where(times > time_sorted[x], one_vec,
                                                                                zero_vec)
        else:
...
def ConHypergraph(data_name, user_size):
    user_size, all_cascade, all_time = Split_data_1(data_name, load_dict=True)
    ###context
    user_cont = {}
